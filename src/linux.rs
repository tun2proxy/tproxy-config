...
async fn detect_routing_loop(proxy_cidr: &IpCidr, tun_name: &str) -> Result<()> {
    ...
    log::debug!("tun ({}) interface id: {}", tun_name, tun_interface_id);
    ...
    if let Some(route) = proxy_route {
        ...
        return Err(std::io::Error::new(std::io::ErrorKind::NotFound, format!("route to proxy ({}) not found", proxy_cidr)).into());
    }
    Ok(())
}
